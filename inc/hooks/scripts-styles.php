...
} // end Cbrne_Theme_move_jquery_into_footer
...
function enqueue_theme_scripts()
{
  ...
  // Required comment-reply script
    ...
    wp_localize_script('scripts', 'Cbrne_Theme_screenReaderText', [
  ...
  // Add domains/hosts to disable external link indicators
    wp_localize_script('scripts', 'Cbrne_Theme_externalLinkDomains', THEME_SETTINGS['external_link_domains_exclude']);
} // end Cbrne_Theme_scripts
...
function enqueue_polyfills()
{
  ...
  if (!supportsES6) {
    ...
    if (file_exists(get_theme_file_path(get_asset_file('legacy.js')))) {
        wp_register_script('Cbrne_Theme_legacy', '', [], filemtime(get_theme_file_path(get_asset_file('legacy.js'))), false);
        wp_enqueue_script('Cbrne_Theme_legacy');
        wp_add_inline_script('Cbrne_Theme_legacy', $script, true);
    }
