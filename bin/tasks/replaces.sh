...
for i in `grep -rl cbrnetheme * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;cbrnetheme;${THEME_NAME};" $i $i; done
for i in `grep -rl cbrnetheme * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;cbrnetheme;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;cbrnetheme;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Cbrne_Theme_;${THEME_NAME}_;" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Cbrne_Theme_;${THEME_NAME}_;" $i $i; done
